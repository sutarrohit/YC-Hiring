...
export default function CompanyCard({ company }: CompanyCardProps) {
  return (
    <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-lg shadow-sm hover:shadow-md hover:border-gray-300 dark:hover:border-neutral-700 transition-all duration-200 flex flex-col h-full overflow-hidden group">
      <div className="p-5 flex-grow">
        ...
        <p className="text-sm text-gray-600 dark:text-neutral-400 mb-4 line-clamp-3">
          {company.one_liner}
        </p>
        
        <div className="flex flex-wrap gap-2 mb-2">
            {company.industries?.slice(0, 2).map((ind) => (
                <span key={ind} className="text-xs text-gray-500 dark:text-neutral-500 bg-gray-100 dark:bg-neutral-800 px-2 py-1 rounded border border-gray-200 dark:border-neutral-700">
                    {ind}
                </span>
            ))}
        </div>
